...
// FIXME Not cache friendly, uses too much auxiliary memory
template <class scalar_t>
void _quantize_median_cut_impl(
    ...
    if (order <= 0) {
        ...
        _quantize_median_cut_impl(data, tmp_rev_map, colors_out,
...
std::tuple<torch::Tensor, torch::Tensor> quantize_median_cut(torch::Tensor data,
                                                             int32_t order) {
    ...
    AT_DISPATCH_FLOATING_TYPES(data.type(), __FUNCTION__, [&] {
        ...
        _quantize_median_cut_impl<scalar_t>(
            comp.data, tmp, colors.accessor<scalar_t, 2>(),
            color_id_map.accessor<int32_t, 1>(), order, 0, data.size(0),
            color_idx, comp);
    });
